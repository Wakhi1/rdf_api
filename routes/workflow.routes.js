    ...
    try {
      ...
      const workflowLevels = [
        ...
        },
        {
          level: 'PS_LEVEL',
          name: 'PS Level',
          description: 'Final approval by Permanent Secretary',
          sequence: 9,
          progress: 72.72
        },
        {
...
router.post('/:applicationId/advance',
  authenticate,
  validateParams({ applicationId: 'number' }),
  async (req, res) => {
    try {
    ...
    try {
      ...
      const committeeMember = await db.getOne(
        `SELECT cm.* FROM committee_members cm
         ...
         WHERE cm.user_id = ? AND c.type = ? AND c.status = 'active'
         AND (
           (c.scope_type = 'national') OR
           (c.scope_type = 'region' AND c.scope_id = ?) OR
           (c.scope_type = 'tinkhundla' AND c.scope_id = ?) OR
           (c.scope_type = 'umphakatsi' AND c.scope_id = ?)
         )`,
        ...
        if (allApproved) {
          const chairperson = await db.getOne(
            `SELECT u.* FROM users u
             ...
             WHERE cm.committee_id = ? AND cm.role = 'Chairperson'
    ...
    try {
      ...
      const committees = await db.query(
        `SELECT c.*, 
         ...
         SUM(CASE WHEN ca.approved = TRUE THEN 1 ELSE 0 END) as approved_count,
         SUM(CASE WHEN ca.approved = FALSE THEN 1 ELSE 0 END) as rejected_count
         FROM committees c
  ...
  if (['UMPHAKATSI_LEVEL', 'INKHUNDLA_LEVEL', 'RDFTC_LEVEL', 'RDFC_LEVEL'].includes(application.current_level)) {
    ...
    const isChairperson = await db.getOne(
      `SELECT cm.id FROM committee_members cm
       JOIN committees c ON c.id = cm.committee_id
       WHERE cm.user_id = ? AND cm.role = 'Chairperson' AND c.type = ? AND c.status = 'active'
       AND (
         (c.scope_type = 'national') OR
         (c.scope_type = 'region' AND c.scope_id = ?) OR
         (c.scope_type = 'tinkhundla' AND c.scope_id = ?) OR
...
async function checkAllCommitteeApprovals(connection, applicationId, committeeType) {
  ...
  const committeeMembers = await connection.query(
    `SELECT cm.id FROM committee_members cm
     JOIN committees c ON c.id = cm.committee_id
     WHERE c.type = ? AND c.status = 'active' AND cm.status = 'active'
     AND (
       (c.scope_type = 'national') OR
       (c.scope_type = 'region' AND c.scope_id = ?) OR
       (c.scope_type = 'tinkhundla' AND c.scope_id = ?) OR
  ...
  const approvals = await connection.query(
    `SELECT ca.* FROM committee_approvals ca
     ...
     AND cm.status = 'active' AND ca.approved = TRUE`,
    ...
    case 'UMPHAKATSI_LEVEL':
      return await db.query(
        `SELECT u.* FROM users u
         JOIN committee_members cm ON cm.user_id = u.id
         JOIN committees c ON c.id = cm.committee_id
         WHERE c.type = 'CDC' AND c.status = 'active'
         AND (c.scope_type = 'umphakatsi' AND c.scope_id = ?)
         AND u.status = 'active'`,
    ...
    case 'INKHUNDLA_LEVEL':
      return await db.query(
        `SELECT u.* FROM users u
         JOIN committee_members cm ON cm.user_id = u.id
         JOIN committees c ON c.id = cm.committee_id
         WHERE c.type = 'INKHUNDLA_COUNCIL' AND c.status = 'active'
         AND (c.scope_type = 'tinkhundla' AND c.scope_id = ?)
         AND u.status = 'active'`,
    ...
    case 'RDFTC_LEVEL':
      return await db.query(
        `SELECT u.* FROM users u
         JOIN committee_members cm ON cm.user_id = u.id
         JOIN committees c ON c.id = cm.committee_id
         WHERE c.type = 'RDFTC' AND c.status = 'active'
         AND (c.scope_type = 'region' AND c.scope_id = ?)
         AND u.status = 'active'`,
    ...
    case 'RDFC_LEVEL':
      return await db.query(
        `SELECT u.* FROM users u
         JOIN committee_members cm ON cm.user_id = u.id
         JOIN committees c ON c.id = cm.committee_id
         WHERE c.type = 'RDFC' AND c.status = 'active'
         AND (c.scope_type = 'region' AND c.scope_id = ?)
         AND u.status = 'active'`,
        [application.region_id]
      );
